...
Si F_\alpha es una aproximación de F entonces lfp(F_\alpha) es una aproximación de lfp(F).
...
- $T_P \uparrow \omega$ es procedimiento para obtener el mínimo punto fijo comenzando desde $\omega = 1$.
...
- Función de abstracción: \alpha
  - $\alpha : I \to I^\alpha$
- Operador abstracto: T^\alpha_P
  - Es la versión abstracta de T_P
...
- Exactitud:
  ...
  - T^\alpha_P es una aproximación segura de T_P
